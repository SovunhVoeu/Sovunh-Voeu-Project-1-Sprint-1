...
def test_user_data_entry_save(second_window):
    if not QSqlDatabase.contains("qt_sql_default_connection"):
        second_window.db = QSqlDatabase.addDatabase("QSQLITE")
    else:
        second_window.db = QSqlDatabase.database("qt_sql_default_connection")

    if not second_window.db.isOpen():
        if not second_window.db.open():
            pytest.fail(f"Failed to open database: {second_window.db.lastError().text()}")
    ...
    assert second_window.db.isOpen(), "Database connection lost before querying"
